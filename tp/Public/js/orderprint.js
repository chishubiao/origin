...
(function($,window) {
    ...
    $.JOP = {
        ...
        bodyhtml:function(t,settings){
            ...
            html += '  <tr><td colspan=3 class="print2" style="text-align:center;"><span style="color:red;font-size:18px;">'+strcaizhong+'</span><span style="font-size:14px;">(第<span id="_printnumber"></span>次打印)</span></td></tr>';
            ...
            $("#numprint").unbind('click').click(function(){ //点击打印
                ...
                if(_printstat==1){
                    if(window.confirm("你已经打印过该票，还要打印吗？")){
                        $.JOP.PrintAjax("打印",'./'+$.JOP.ajaxurl+'?action=memberprintstat'+_oidstr+'',{"inajax":1,"versionName":$.JOP.versionName,"iCurrPage":$.JOP.iPage,"sid":""+settings._sid+"","strid":""+strid+"","bscount":""+_bscount+"","orderpagestat":""+orderpagestat+""},function (){
        ...
        ,but_start:function(){
            $("#numprint").attr('disabled',false);
            $("#numprint").val('打 印');

        }
        ,but_stop:function(){
            $("#numprint").attr('disabled',true);
            $("#numprint").val('打印中');
        }
        ,focusnumber:function(){

            if ($.JOP.IN_WAPCG === 0) {
                var soonObj = window.parent.frames["main"].frames["soonorder"];
                if(soonObj && soonObj.document.getElementById("number")!=null)soonObj.document.getElementById("number").select();
        ...
        orderdata:function(){
            ...
            //console.log($.JOP.printnum+'===' + pagenum+'===' + joinstr['j']+'===' + newstat);
            ...
            $("#_printnumber").text((_printnumber+1));
        ...
        PrintAjax:function (tit, url ,data ,callbackFunction,beforeSend,complete){
            $.ajax( {
                url:url,
                data:data,
                type:'post',
